...
### Nodos de ajuste (covariables, padres de los biomarcadores)
- `h0303` — Edad: riesgo de diabetes aumenta con la edad.  
- `h0302` — Sexo: diferencias biológicas en metabolismo.  
- `h0317a/g` — Escolaridad: nivel educativo como proxy socioeconómico.  
- `region`, `estrato` — Localización urbano-rural que determina exposición ambiental.  
...
- `Edad`, `Sexo`, `Escolaridad`, `Región/Estrato` → {`HB1AC`, `GLU_SUERO`, `INSULINA`}  
...
```{r}
data_dag_inf <- data2 |> 
  select(NOx, PM_2_5, valor_PROTCREAC,valor_PCR,valor_HB1AC,valor_INSULINA,valor_GLU_SUERO, h0303,
         h0317a, h0317g, region, estrato) |> drop_na()
```


```{r}
colnames(data_dag_inf) = c("N","P", "Gc","CRP", "Hb", "I","Ga","Edad","EdNiv","EdGrad","Reg","Est")
view(data_dag_inf)

```

```{r}
cat_vars <- c("EdNiv", "EdGrad", "Reg", "Est")
data_dag_inf[cat_vars] <- lapply(data_dag_inf[cat_vars], as.factor)
```

```{r}
library(bnlearn)

dag2 = model2network("[N][P][Edad][EdNiv][EdGrad|EdNiv][Est][Reg|Est][Gc|P:N][I|P:N][Ga|P:N][Hb|I:Ga:P:N][CRP|Gc:P:N:Edad]")
